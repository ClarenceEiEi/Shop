<%-- 
    Document   : Login
    Created on : Sep 11, 2017, 2:23:23 AM
    Author     : PUNEIEI
--%>
...
<body>
    <%
           ...
           Login log = new Login();
          // log.Action = request.getParameter("Action");
	...
	if(log.getAction()!= null)	
	{
			...
			//log.username = request.getParameter("txtUsername");
			//log.password = request.getParameter("txtPassword");	                       			
			//con.s = con.connect.createStatement();    			
                        
                        log.setTxPass(request.getParameter("txtPassword"));
                        log.setTxUser(request.getParameter("txtUsername"));
                        log.setlogin(log.getTxUser(),log.getTxPass());
                        log.rec = con.s.executeQuery(log.login);  
                       
			if(!log.rec.next())
			{
				out.print("<font color = red>"+log.PTlog(true)+"</font>");%>
                                <br>
                                <br>
                                <%
			} else {
				log.rec.first();
				session.setAttribute("sUserID",log.rec.getString("ID"));
                                session.setAttribute("UN",log.rec.getString("Username"));
  ...
  <div class="login" data-loginnaja="{sessionScope.log}" id="logg">
	<h1>Login</h1>
    <form method="post">
    	<input type="text" name="txtUsername" placeholder="Username" required="required" type="text" id="txtUsername" />
        <input type="password" name="txtPassword" placeholder="Password" required="required"  type="password" id="txtPassword" />
        ...
        <form  action="register.jsp">                 
            <br>
                 ...
                 <button width="20" type="submit"  name="Submit" value="Register" 
                         class="btn btn-primary btn-block btn-large">Register</button>   
